logspaceeplot=function(X,k=trunc(length(X)/10),grid=seq(0.01,0.99,0.01),weighted=T,add.line=F){
  ...
  if(weighted==T){
    plot(y=expect((1:k)*log(quantile(X,1-(1:k-1)/n,type=1)/quantile(X,1-1:k/n,type=1)),grid),x=eexp(grid),pch=16,main="EE-plot",ylab="Log-spacings expectiles",xlab="Exponential expectiles")
  }
  if(weighted==F){
